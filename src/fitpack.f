...
c
      ...
      REAL(r8), DIMENSION(n) :: x,y,yp,temp
      REAL(r8) :: slp1,slpn,sigma

      REAL(r8) :: c1,c2,c3,delx1,delx2,delxn,delxnm,diag,diag1,diag2
      REAL(r8) :: dx1,dx2,sdiag1,sdiag2,sigmap,slpp1,slppn
      INTEGER(i4) :: i,ibak,nm1,np1
c
...
c     sigmap = abs(sigma)*float(n-1)/(x(n)-x(1))
      sigmap = abs(sigma)*REAL((n-1),r8)/(x(n)-x(1))
c
...
c
      ...
      INTEGER(i4) :: n,isw,ierr
      REAL(r8), DIMENSION(n) :: x,y,d,ys,ysp
      REAL(r8), DIMENSION(n,9) :: temp
      REAL(r8) :: s,eps,sigma
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
...
c
      ...
      INTEGER(i4) :: n
      REAL(r8) :: t,x(n),y(n),yp(n),sigma

      REAL(r8) :: del1,del2,dels,dummy,s1,s2,sigdel,sigmap,ss,sum
      INTEGER(i4) :: i,im1
...
c
      ...
      REAL(r8) :: c1,c2,del1,del2,dels,dummy,sigdel,sigmap,ss,sum
      INTEGER(i4) :: i,im1
...
c
      ...
      sum = (y(i)-y(im1))/dels
      if (sigmap .ne. 0.) go to 1
      curvd = sum+(yp(i)*(2._r8*del1*del1-del2*(del1+dels))-
     *             yp(im1)*(2._r8*del2*del2-del1*(del2+dels)))
     *             /(6._r8*dels)
      return
    1 sigdel = sigmap*dels
      call snhcsh_1 (ss,dummy,sigdel,-1)
...
c
      ...
      INTEGER(i4) :: n
      REAL(r8) :: xl,xu,x(n),y(n),yp(n),sigma
      REAL(r8) :: curvi

      REAL(r8) :: c1,c2,cl1,cl2,cs,cu1,cu2,del1,del2,deli,dell1,dell2
      REAL(r8) :: dels,delu1,delu2,dummy,sigmap,ss,ssignsum,t1,t2,xxl
      REAL(r8) :: xxu,ssign,sum
      INTEGER(i4) :: i,il,ilm1,ilp1,iu,ium1
...
c
      ...
      del2 = x(il)-xxl
      dels = x(il)-x(ilm1)
      t1 = (del1+dels)*del2/(2._r8*dels)
      t2 = del2*del2/(2._r8*dels)
      ...
      sum = sum+((dels*dels*(cs-ss/2._r8)-del1*del1*(c1-ss/2._r8))
     *           *yp(il)+del2*del2*(c2-ss/2._r8)*yp(ilm1))/
     *          (sigmap*sigmap*dels*(1._r8+ss))
      go to 3
    2 sum = sum-t1*t1*dels*yp(il)/6._r8
     *         -t2*(del1*(del2+dels)+dels*dels)*yp(ilm1)/12._r8
c
...
c
    3 if (iu-il .eq. 1) go to 6
      ...
      do 5 i = ilp1,ium1
        dels = x(i)-x(i-1)
        sum = sum+(y(i)+y(i-1))*dels/2._r8
        if (sigma .eq. 0.) go to 4
        call snhcsh_1 (ss,cs,sigmap*dels,3)
        sum = sum+(yp(i)+yp(i-1))*dels*(cs-ss/2._r8)/
     *            (sigmap*sigmap*(1._r8+ss))
        go to 5
    4   sum = sum-(yp(i)+yp(i-1))*dels*dels*dels/24._r8
    5   continue
c
...
c
    6 if (xxu .eq. x(ium1)) go to 10
      ...
      del2 = x(iu)-xxu
      dels = x(iu)-x(ium1)
      t1 = del1*del1/(2._r8*dels)
      t2 = (del2+dels)*del1/(2._r8*dels)
      sum = sum+t1*y(iu)+t2*y(ium1)
      if (sigma .eq. 0.) go to 7
      call snhcsh_1 (dummy,c1,sigmap*del1,2)
      call snhcsh_1 (dummy,c2,sigmap*del2,2)
      call snhcsh_1 (ss,cs,sigmap*dels,3)
      sum = sum+(yp(iu)*del1*del1*(c1-ss/2._r8)+yp(ium1)*
     *          (dels*dels*(cs-ss/2._r8)-del2*del2*(c2-ss/2._r8)))
     *         /(sigmap*sigmap*dels*(1._r8+ss))
      go to 10
    7 sum = sum-t1*(del2*(del1+dels)+dels*dels)*yp(iu)/12._r8
     *         -t2*t2*dels*yp(ium1)/6._r8
      go to 10
c
...
c
    8 delu1 = xxu-x(ium1)
      ...
      dels = x(iu)-x(ium1)
      deli = xxu-xxl
      t1 = (delu1+dell1)*deli/(2._r8*dels)
      t2 = (delu2+dell2)*deli/(2._r8*dels)
      ...
      sum = sum+(yp(iu)*(delu1*delu1*(cu1-ss/2._r8)
     *            -dell1*dell1*(cl1-ss/2._r8))
     *          +yp(ium1)*(dell2*dell2*(cl2-ss/2._r8)
     *            -delu2*delu2*(cu2-ss/2._r8)))/
     *          (sigmap*sigmap*dels*(1._r8+ss))
      go to 10
...
c
      ...
      INTEGER(i4) :: n,ierr
      REAL(r8), DIMENSION(n) :: x,y,yp
      REAL(r8), DIMENSION(1) :: temp
      REAL(r8) :: p,sigma

      REAL(r8) :: delx1,delx2,diag,diag1,diag2,dx1,dx2,sdiag1,sdiag2
      REAL(r8) :: sigmap,ypn
      INTEGER(i4) :: i,ibak,nm1,np1,npi,npibak
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c denormalize tension factor
c
c     sigmap = abs(sigma)*float(n)/p
      sigmap = abs(sigma)*REAL(n,r8)/p
c
c set up right hand side and tridiagonal system for yp and
c perform forward elimination
c
...
c
      INTEGER(i4) :: n,isw,ierr
      REAL(r8), DIMENSION(n) :: x,y,d,ys,ysp
      REAL(r8), DIMENSION(n,11) :: temp
      REAL(r8) :: p,s,eps,sigma
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c
      ...
      INTEGER(i4) :: n
      REAL(r8) :: t,x(n),y(n),p,yp(n),sigma
      REAL(r8) :: curvp2

      REAL(r8) :: del1,del2,dels,dummy,s1,s2,sigdel,sigmap,ss,sum,tp
      INTEGER(i4) :: i,im1
...
c
      ...
      INTEGER(i4) :: n
      REAL(r8) :: xl,xu,x(n),y(n),p,yp(n),sigma
      REAL(r8) :: curvpi

      REAL(r8) :: c1,c2,cl1,cl2,cs,cu1,cu2,del1,del2,deli,dell1,dell2
      REAL(r8) :: dels,delu1,delu2,dummy,s1,s2,s3,s4,s5,s6,s7,s8,si
      REAL(r8) :: sigmap,so,ss,t1,t2,x1pp,xil,xiu,xsave,xxl,xxu
      INTEGER(i4) :: i,ideltp,ii,il,ilm1,ilp1,im1,isave,isigni,iu,ium1
      INTEGER(i4) :: iup1,lper,np1,isign
...
c
      ...
      ideltp = uper-lper
c     bdy = float(ideltp)*(xxu-xxl) .lt. 0.
      bdy = REAL(ideltp,r8)*(xxu-xxl) .lt. 0.
...
c
      do 3 i = 2,ilm1
        dels = x(i)-x(i-1)
        s1 = s1+(y(i)+y(i-1))*dels/2._r8
        if (sigma .eq. 0.) go to 2
        call snhcsh_1 (ss,cs,sigmap*dels,3)
        s1 = s1+(yp(i)+yp(i-1))*dels*(cs-ss/2._r8)/
     *            (sigmap*sigmap*(1._r8+ss))
        go to 3
    2   s1 = s1-(yp(i)+yp(i-1))*dels*dels*dels/24._r8
    3   continue
    4 s2 = 0._r8
...
c
      ...
      call snhcsh_1 (dummy,c2,sigmap*del2,2)
      call snhcsh_1 (ss,cs,sigmap*dels,3)
      s2 = s2+(yp(il)*del1*del1*(c1-ss/2._r8)+yp(ilm1)*
     *          (dels*dels*(cs-ss/2._r8)-del2*del2*(c2-ss/2._r8)))
     *         /(sigmap*sigmap*dels*(1._r8+ss))
      go to 6
    5 s2 = s2-t1*(del2*(del1+dels)
     *     +dels*dels)*yp(il)/12._r8
     *     -t2*t2*dels*yp(ilm1)/6._r8
    6 s3 = 0._r8
...
c
      ...
      call snhcsh_1 (dummy,c2,sigmap*del2,2)
      call snhcsh_1 (ss,cs,sigmap*dels,3)
      s3 = s3+((dels*dels*(cs-ss/2._r8)-del1*del1*(c1-ss/2._r8))
     *           *yp(il)+del2*del2*(c2-ss/2._r8)*yp(ilm1))/
     *          (sigmap*sigmap*dels*(1._r8+ss))
      go to 8
    7 s3 = s3-t1*t1*dels*yp(il)/6._r8
...
c
      ...
      do 10 i = ilp1,ium1
        dels = x(i)-x(i-1)
        s4 = s4+(y(i)+y(i-1))*dels/2._r8
        if (sigma .eq. 0.) go to 9
        call snhcsh_1 (ss,cs,sigmap*dels,3)
        s4 = s4+(yp(i)+yp(i-1))*dels*(cs-ss/2._r8)/
     *            (sigmap*sigmap*(1._r8+ss))
        go to 10
    9   s4 = s4-(yp(i)+yp(i-1))*dels*dels*dels/24._r8
   10   continue
   11 s5 = 0._r8
...
c
      ...
      call snhcsh_1 (dummy,c2,sigmap*del2,2)
      call snhcsh_1 (ss,cs,sigmap*dels,3)
      s5 = s5+(yp(iu)*del1*del1*(c1-ss/2._r8)+yp(ium1)*
     *          (dels*dels*(cs-ss/2._r8)-del2*del2*(c2-ss/2._r8)))
     *         /(sigmap*sigmap*dels*(1._r8+ss))
      go to 13
   12 s5 = s5-t1*(del2*(del1+dels)
     *     +dels*dels)*yp(iu)/12._r8
     *     -t2*t2*dels*yp(ium1)/6._r8
   13 s6 = 0._r8
...
c
      ...
      call snhcsh_1 (dummy,c2,sigmap*del2,2)
      call snhcsh_1 (ss,cs,sigmap*dels,3)
      s6 = s6+((dels*dels*(cs-ss/2._r8)-del1*del1*(c1-ss/2._r8))
     *           *yp(iu)+del2*del2*(c2-ss/2._r8)*yp(ium1))/
     *          (sigmap*sigmap*dels*(1._r8+ss))
      go to 15
   14 s6 = s6-t1*t1*dels*yp(iu)/6._r8
     *     -t2*(del1*(del2+dels)+dels*dels)*
     *     yp(ium1)/12._r8
   15 s7 = 0._r8
...
c
      ...
      do 17 ii = iup1,np1
        ...
        s7 = s7+(y(i)+y(im1))*dels/2._r8
        if (sigma .eq. 0.) go to 16
        call snhcsh_1 (ss,cs,sigmap*dels,3)
        s7 = s7+(yp(i)+yp(im1))*dels*(cs-ss/2._r8)/
     *            (sigmap*sigmap*(1._r8+ss))
        go to 17
   16   s7 = s7-(yp(i)+yp(im1))*dels*dels*dels/24._r8
   17   continue
   18 s8 = 0._r8
...
c
      ...
      call snhcsh_1 (dummy,cl2,sigmap*dell2,2)
      call snhcsh_1 (ss,dummy,sigmap*dels,-1)
      s8 = s8+(yp(iu)*(delu1*delu1*(cu1-ss/2._r8)
     *            -dell1*dell1*(cl1-ss/2._r8))
     *          +yp(ium1)*(dell2*dell2*(cl2-ss/2._r8)
     *            -delu2*delu2*(cu2-ss/2._r8)))/
     *          (sigmap*sigmap*dels*(1._r8+ss))
   ...
   19 s8 = s8-t1*(delu2*(dels+delu1)
     *     +dell2*(dels+dell1))*yp(iu)/12._r8
     *     -t2*(dell1*(dels+dell2)
     *     +delu1*(dels+delu2))*yp(ium1)/12._r8
   20 so = s1+s2+s6+s7
      si = s3+s4+s5+s8
      if (bdy) go to 21
c     curvpi = float(ideltp)*(so+si)+float(isign)*si
      curvpi = REAL(ideltp,r8)*(so+si)+REAL(isign,r8)*si
      return
c  21 curvpi = float(ideltp)*(so+si)+float(isign)*so
   21 curvpi = REAL(ideltp,r8)*(so+si)+REAL(isign,r8)*so
      return
      end
c
c************************************************************
...
c
      INTEGER(i4) :: n,islpsw,ierr
      REAL(r8), DIMENSION(n) :: x,y,xp,yp,temp,s
      REAL(r8) :: slp1,slpn,sigma

      REAL(r8) :: c1,c2,c3,dels1,dels2,delsn,delsnm,delt,diag,diag1
      REAL(r8) :: diag2,diagin,dx1,dx2,dy1,dy2,sdiag1,sdiag2,sigmap
      REAL(r8) :: slpp1x,slpp1y,slppnx,slppny,sx,sy
      INTEGER(i4) :: i,ibak,im1,nm1,np1
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c denormalize tension factor
c
c     sigmap = abs(sigma)*float(n-1)/s(n)
      sigmap = abs(sigma)*REAL(n-1,r8)/s(n)
c
c approximate end slopes
c
...
c
      INTEGER(i4) :: n
      REAL(r8), DIMENSION(n) :: x,y,xp,yp,s
      REAL(r8) :: t,xs,ys,sigma

      REAL(r8) :: c1,c2,d,del1,del2,dels,dummy,s1,s2,sigdel,sigmap,ss
      REAL(r8) :: sumx,sumy,tn
      INTEGER(i4) :: i,im1
      INTEGER(i4) :: intrvl_1
c
c                                 coded by alan kaylor cline
...
c
      INTEGER(i4) :: n
      REAL(r8), DIMENSION(n) :: x,y,xp,yp,s
      REAL(r8) :: t,xs,ys,xst,yst,xstt,ystt,sigma

      REAL(r8) :: c1,c2,co1,co2,ct1,ct2,ctt1,ctt2,d,del1,del2,dels,dels6
      REAL(r8) :: dummy,s1,s2,sigdel,sigmap,ss,sumx,sumxt,sumy,sumyt,tn
      INTEGER(i4) :: i,im1
...
c
      ...
      c2 = -(del2+dels)*d
      dels6 = dels6/s(n)
      ct1 = (2._r8*del1*del1-del2*(del1+dels))/dels6
      ct2 = -(2._r8*del2*del2-del1*(del2+dels))/dels6
      dels = dels/(s(n)*s(n))
      ctt1 = del1/dels
      ctt2 = del2/dels
      go to 2
    1 sigdel = sigmap*dels
      ...
      ct1 = (co1-ss)*s(n)/d
      ct2 = -(co2-ss)*s(n)/d
      ctt1 = del1*(1._r8+s1)*s(n)*s(n)/(dels*(1._r8+ss))
      ctt2 = del2*(1._r8+s2)*s(n)*s(n)/(dels*(1._r8+ss))
...
c
      ...
      REAL(r8) :: dels1,dels2,diag,diag1,diag2,diagin,dx1,dx2,dy1,dy2
      REAL(r8) :: sdiag1,sdiag2,sigmap,xpn,ypn
      INTEGER(i4) :: i,ibak,im1,nm1,np1,npi,npibak
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c denormalize tension factor
c
c     sigmap = abs(sigma)*float(n)/s(n)
      sigmap = abs(sigma)*REAL(n,r8)/s(n)
c
...
c
      ...
      REAL(r8) :: c1,c2,ci,cim1,d,del1,del2,dels,dummy,s1,s2,si,sigdel
      REAL(r8) :: sigmap,ss,sumx,sumy,tn
      INTEGER(i4) :: i,im1
...
c
      tn = t-real(int(t))
      if (tn .lt. 0.) tn = tn+1._r8
      tn = s(n)*tn+s(1)
      im1 = n
      if (tn .lt. s(n)) im1 = intrvl_1(tn,s,n)
      i = im1+1
      if (i .gt. n) i = 1
c
c denormalize tension factor
...
c
      ...
      REAL(r8) :: c1,c2,co1,co2,ct51,ct2,ctt1,ctt2,d,del1,del2,dels
      REAL(r8) :: dels6,dummy,s1,s2,si,sigdel,sigmap,ss,sumx,sumxt
      REAL(r8) :: ct1,sumy,sumyt,tn
      INTEGER(i4) :: i,im1
...
c
      ...
      IMPLICIT NONE
c
      INTEGER(i4) :: m,n,iz,islpsw,ierr
      REAL(r8), DIMENSION(m) :: x,zy1,zyn
      REAL(r8), DIMENSION(n) :: y,zx1,zxm
      REAL(r8), DIMENSION(m,n,3) :: zp
      REAL(r8), DIMENSION(iz,n) :: z
      REAL(r8), DIMENSION(1) :: temp
      REAL(r8) :: zxy11,zxym1,zxy1n,zxymn,sigma

      REAL(r8) :: c1,c2,c3,del1,del2,deli,delx1,delx2,delxm,delxmm,dely1
      REAL(r8) :: dely2,delyn,delynm,diag1,diag2,diagi,diagin,sdiag2
      REAL(r8) :: sdiag1,sigmax,sigmay,t,zxy1ns,zxymns
      INTEGER(i4) :: i,ibak,ibakp1,im1,ip1,j,jbak,jbakp1,jm1,jp1,mm1,mp1
      INTEGER(i4) :: nm1,np1,npi,npibak,npm,npmpj
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c
c     sigmay = abs(sigma)*float(n-1)/(y(n)-y(1))
      sigmay = abs(sigma)*REAL(n-1,r8)/(y(n)-y(1))
c
...
c
c     sigmax = abs(sigma)*float(m-1)/(x(m)-x(1))
      sigmax = abs(sigma)*REAL(m-1,r8)/(x(m)-x(1))
c
...
c
   ...
   31     zp(i,jp1,1) = deli*(z(i,jp1)-z(i,j))
        ...
        diagin = 1._r8/(diag1+diag2-sdiag1*temp(jm1))
        do 32 i = 1,m
   32     zp(i,j,1) = diagin*(zp(i,jp1,1)-zp(i,j,1)-
     ...
     *                      sdiag1*zp(m,jm1,3))
        ...
        diag1 = diag2
   33   sdiag1 = sdiag2
   34 diagin = 1._r8/(diag1-sdiag1*temp(nm1))
      do 35 i = 1,m
        npi = n+i
...
c
   ...
   40     zp(ip1,j,3) = deli*(zp(ip1,j,1)-zp(i,j,1))
        ...
        diagin = 1._r8/(diag1+diag2-sdiag1*temp(npi-1))
        do 41 j = 1,n
   ...
   41     zp(i,j,3) = diagin*(zp(ip1,j,3)-zp(i,j,3)-
     *                        sdiag1*zp(im1,j,3))
        ...
        diag1 = diag2
   42   sdiag1 = sdiag2
   43 diagin = 1._r8/(diag1-sdiag1*temp(npm-1))
      do 44 j = 1,n
        npmpj = npm+j
...
c
      ...
      INTEGER(i4) :: m,n,iz
      REAL(r8) :: xx,yy,x(m),y(n),z(iz,n),zp(m,n,3),sigma
      REAL(r8) :: surf2

      REAL(r8) :: f1,f2,fp1,fp2,sigmap,del1,del2,dels,dummy,sigmax
      REAL(r8) :: sigmay,sinhm1,sinhm2,sinhms,zi,zim1,zxxi,zxxim1
      REAL(r8) :: hermnz,hermz
      INTEGER(i4) :: i,im1,j,jm1
...
c
     ...
     *          )/(sigmap*sigmap*dels*(1._r8+sinhms))
c
...
c
     ...
     *              zp(i,j-1,3),zp(i,j,3))
      go to 2
    1 call snhcsh_1 (sinhm1,dummy,sigmay*del1,-1)
...
c
    2 del1 = xx-x(im1)
      ...
      return
    3 call snhcsh_1 (sinhm1,dummy,sigmax*del1,-1)
...
c
      ...
      IMPLICIT NONE
c
      REAL(r8) :: del1,del2,sigma,c1,c2,c3
      INTEGER(i4) :: n

      REAL(r8) :: coshm1,coshm2,del,delm,delp,denom,dummy,sinhmm,sinhmp
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c tension .ne. 0.
c
    1 call snhcsh_1 (dummy,coshm1,sigma*del1,1)
      call snhcsh_1 (dummy,coshm2,sigma*del2,1)
      delp = sigma*(del2+del1)/2._r8
      delm = sigma*(del2-del1)/2._r8
      call snhcsh_1 (sinhmp,dummy,delp,-1)
      call snhcsh_1 (sinhmm,dummy,delm,-1)
      denom = coshm1*(del2-del1)-2._r8*del1*delp*delm*
     *        (1._r8+sinhmp)*(1._r8+sinhmm)
      c1 = 2._r8*delp*delm*(1._r8+sinhmp)*(1._r8+sinhmm)/denom
      c2 = -coshm2/denom
      c3 = coshm1/denom
      return
c
...
c
      INTEGER(i4) :: n,isw,ierr
      REAL(r8), DIMENSION(n) :: x,y,d,ys,ysp,td,tsd1,hd,hsd1,hsd2,rd
      REAL(r8), DIMENSION(n) :: rsd1,rsd2,rnm1,rn,v
      REAL(r8) :: p,s,eps,sigma

      REAL(r8) :: alpha,alphap,beta,betap,betapp,con,delxi,delxi1,delyi1
      REAL(r8) :: di,dim1,disq,f,g,h,hdi,hdim1,hsd11,hsd1ip,hsd1p,delyi
      REAL(r8) :: q,rdn,rnm1sm,rnm1t,rnsmrnt,rsd1i,rsd2i,sigmap,sl,step
      REAL(r8) :: su,sum,sum2,sumd,sumn,sumnm1,sumy,tui,wi,wim1,rnsm,rnt
      REAL(r8) :: wim2,yspn,yspnm1
      INTEGER(i4) :: i,ibak,im1,ip1,nm1,nm2,nm3
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c
      ...
      do 1 i = 1,n
        ...
        td(i) = di+dim1
        hd(i) = -(1._r8/delxi+1._r8/delxi1)
        hsd1(ip1) = 1._r8/delxi
        delxi1 = delxi
        delyi1 = delyi
    1   dim1 = di
      hsd11 = hsd1(1)
...
c i = 2
c
      rd(2) = 1._r8/(q*td(2)+hd(2)-rsd1i*rsd1(2))
      ...
      do 9 i = 3,nm2
        ...
        rsd2(i) = rsd2i*rd(i-2)
        rsd1(i) = rsd1i*rd(i-1)
        rd(i) = 1._r8/(q*td(i)+hd(i)-rsd1i*rsd1(i)
     *                           -rsd2i*rsd2(i))
...
c i = n-1
c
   12 rd(nm1) = 1._r8/(q*td(nm1)+hd(nm1)-sumnm1)
      ysp(nm1) = yspnm1
...
c
      INTEGER(i4) :: n,isw,ierr
      REAL(r8), DIMENSION(n) :: x,y,d,ys,ysp,td,tsd1,hd,hsd1,hsd2,rd
      REAL(r8), DIMENSION(n) :: rsd1,rsd2,v
      REAL(r8) :: s,eps,sigma

      REAL(r8) :: alpha,alphap,beta,betap,betapp,delxi,delxi1,delyi
      REAL(r8) :: delyi1,di,dim1,f,g,h,hdi,hdim1,dsh1p,p,rdim1,rsd1i
      REAL(r8) :: rsd2i,hsd1p,sigmap,sl,step,su,sum,tui,wi,wim1,wim2
      REAL(r8) :: yspim2
      INTEGER(i4) :: i,ibak,nm1,nm3
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c
    5 do 6 i = 2,nm1
        ...
        rdim1 = rd(i-1)
        rsd1(i) = rsd1i*rdim1
        rd(i) = 1._r8/(p*td(i)+hd(i)-rsd1i*rsd1(i)
     *                           -rsd2i*rsd2(i))
...
c
      INTEGER(i4) :: isw
      REAL(r8) :: sinhm,coshm,x

      REAL(r8) :: ax,cp1,cp2,cp3,cp4,cp5,cq0,cq1,cq2,expx,sp11,sp12,sp13
      REAL(r8) :: sp14,sp21,sp22,sp23,sp24,sp25,sp31,sp32,sp33,sp34,sp35
      REAL(r8) :: sp41,sp42,sp43,sp44,sp45,sq10,sq11,sq12,sq20,sq21,sq22
      REAL(r8) :: sq30,sq31,sq32,sq40,sq41,sq42,xs,zp1,zp2,zp3,zp4,zq0
      REAL(r8) :: zq1,zq2
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
...
c-----------------------------------------------------------
c
      data sp14/.227581660976348e-7_r8/,
     *     sp13/.612189863171694e-5_r8/,
     *     sp12/.715314759211209e-3_r8/,
     *     sp11/.398088289992973e-1_r8/,
     *     sq12/.206382701413725e-3_r8/,
     *     sq11/-.611470260009508e-1_r8/,
     *     sq10/.599999999999986e+1_r8/
      data sp25/.129094158037272e-9_r8/,
     *     sp24/.473731823101666e-7_r8/,
     *     sp23/.849213455598455e-5_r8/,
     *     sp22/.833264803327242e-3_r8/,
     *     sp21/.425024142813226e-1_r8/,
     *     sq22/.106008515744821e-3_r8/,
     *     sq21/-.449855169512505e-1_r8/,
     *     sq20/.600000000268619e+1_r8/
      data sp35/.155193945864942e-9_r8/,
     *     sp34/.511529451668737e-7_r8/,
     *     sp33/.884775635776784e-5_r8/,
     *     sp32/.850447617691392e-3_r8/,
     *     sp31/.428888148791777e-1_r8/,
     *     sq32/.933128831061610e-4_r8/,
     *     sq31/-.426677570538507e-1_r8/,
     *     sq30/.600000145086489e+1_r8/
      data sp45/.188070632058331e-9_r8/,
     *     sp44/.545792817714192e-7_r8/,
     *     sp43/.920119535795222e-5_r8/,
     *     sp42/.866559391672985e-3_r8/,
     *     sp41/.432535234960858e-1_r8/,
     *     sq42/.824891748820670e-4_r8/,
     *     sq41/-.404938841672262e-1_r8/,
     *     sq40/.600005006283834e+1_r8/
      data cp5/.552200614584744e-9_r8/,
     *     cp4/.181666923620944e-6_r8/,
     *     cp3/.270540125846525e-4_r8/,
     *     cp2/.206270719503934e-2_r8/,
     *     cp1/.744437205569040e-1_r8/,
     *     cq2/.514609638642689e-4_r8/,
     *     cq1/-.177792255528382e-1_r8/,
     *     cq0/.200000000000000e+1_r8/
      data zp4/.664418805876835e-8_r8/,
     *     zp3/.218274535686385e-5_r8/,
     *     zp2/.324851059327161e-3_r8/,
     *     zp1/.244515150174258e-1_r8/,
     *     zq2/.616165782306621e-3_r8/,
     *     zq1/-.213163639579425e0_r8/,
     *     zq0/.240000000000000e+2_r8/
c
      ax = abs(x)
      if (isw .ge. 0) go to 5
c
...
c
      sinhm = xs*((((sp14*xs+sp13)*xs+sp12)*xs+sp11)*xs+1._r8)/
...
c
     ...
     .        *xs+1._r8)/((sq42*xs+sq41)*xs+sq40)
      return
c
c sinhm approximation above 6.1
c
    4 expx = exp(ax)
      sinhm = (expx-1._r8/expx)/(ax+ax)-1._r8
      return
c
...
c
    5 if (isw .ge. 2) go to 7
     ...
     .        *xs+1._r8)/((cq2*xs+cq1)*xs+cq0)
      if (isw .eq. 0) sinhm = xs*((((sp14*xs+sp13)*xs+sp12)
     .          *xs+sp11)*xs+1._r8)/((sq12*xs+sq11)*xs+sq10)
      return
    6 expx = exp(ax)
      coshm = (expx+1._r8/expx)/2._r8-1._r8
      if (isw .eq. 0) sinhm = (expx-1._r8/expx)/(ax+ax)-1._r8
      return
c
...
c
    7 xs = ax*ax
      ...
      coshm = xs*((((zp4*xs+zp3)*xs+zp2)*xs+zp1)*xs+1._r8)/
     .             ((zq2*xs+zq1)*xs+zq0)
      if (isw .eq. 3) sinhm = xs*((((sp14*xs+sp13)*xs+sp12)
     .          *xs+sp11)*xs+1._r8)/((sq12*xs+sq11)*xs+sq10)
      return
    8 expx = exp(ax)
      coshm = ((expx+1._r8/expx-xs)/2._r8-1._r8)/xs
      if (isw .eq. 3) sinhm = (expx-1._r8/expx)/(ax+ax)-1._r8
      return
      end
c
c************************************************************
...
c
      ...
      IMPLICIT NONE
c
      REAL(r8) :: diag,sdiag,sigma,del

      REAL(r8) :: coshm,denom,sigdel,sinhm
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c
    ...
    1 sigdel = sigma*del
      ...
      denom = sigma*sigdel*(1._r8+sinhm)
